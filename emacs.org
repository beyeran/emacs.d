...
#+BEGIN_SRC emacs-lisp :tangle src/jsgf-mode.el
  ...
  (defun goto-last-indent-sign ()
    "In contrast to GOTO-LAST-RELEVANT-INDENT-SIGN, this function only jumps to
  the last sign from the current cursor position."
    (search-backward-regexp *indent-regex*))

  (defun definition-with-linebreak-p ()
    "Checks if the last sign was only followed by an empty string."
    (save-excursion
      (goto-last-indent-sign)
      (forward-char)
      (skip-syntax-forward "-")
      (= (point-at-eol) (point))))

  (defun equal-was-last-p ()
    "Checks if the last sign was an equal sign."
    (save-excursion
      (goto-last-relevant-indent-sign)
      (char-equal (char-before) ?=)))

  (defun pipe-was-last-p ()
    "Checks if the last sign was a pipe symbol."
    (save-excursion
      (goto-last-relevant-indent-sign)
      (char-equal (char-before) ?|)))

  (defun slash-was-last-p ()
    "Checks if the last sign was a slash."
    (save-excursion
      (search-backward-regexp *indent-regex*)
      (char-equal (char-after) ?/)))

  (defun semicolon-was-last-p ()
    "Checks if the last sign was a semicolon."
    (save-excursion
      (goto-last-indent-sign)
      (char-equal (char-after) ?\;)))

  (defun this-line-begins-with-slash-p ()
    "Checks if the current line begins with a slash."
    (save-excursion
      (beginning-of-line)
      (looking-at "^[ \t]\*/")))
  ...
  (defun jsgf-find-ruledef-indent ()
    (cond
     ;; Reset indentation if the last sign was a semicolon.
     ((semicolon-was-last-p) 0)
     ;; Indent the same, if the last sign was a pipe.
     ((pipe-was-last-p) (progn
                          (save-excursion
                            (search-backward-regexp "|")
                            (current-column))))
     ;; Indentation if the last sign was an equal sign.
     ((equal-was-last-p) (progn
                           (save-excursion
                             (let ((begins-with-slash (this-line-begins-with-slash-p)))
                               (cond
                                ;; Standard indent + 2, if this line begins with a slash
                                (begins-with-slash (+ jsgf-tab-width 2))
                                ;; Standard indent if the former line was empty after
                                ;; the equal sign
                                ((definition-with-linebreak-p) jsgf-tab-width)
                                ;; adjust to the level of the equal sign otherwise
                                (t (progn
                                     (search-backward-regexp "=")
                                     (current-column))))))))
     ;; Remove the extra indent of the slash, if there was a slash before.
     ((slash-was-last-p) (progn
                           (save-excursion
                             (goto-last-relevant-indent-sign)
                             (- (current-column) 3))))
