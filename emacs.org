...
** Helping Functions
   ...
   Two macros to specify certain things for only windows or linux systems.
...
#+END_SRC
** Lua
   I'm using only the Lua framework "Torch" for my master's thesis. Despite of
   that, I don't use lua:

#+BEGIN_SRC emacs-lisp src/torch.el
  (on-linux
   (setq lua-default-application "/home/beyeran/opt/torch/install/bin/th")

   (define-key lua-mode-map (kbd "C-c C-r") 'lua-send-region)
   (define-key lua-mode-map (kbd "C-c C-c") 'lua-send-defun))
#+END_SRC
* Looks
** Color Theme
